...
impl NvidiaGpuController {
    ...
    fn apply_xbar_ratio(&self, percent: i32) -> anyhow::Result<()> {
        ...
        // A card can stop offering the ratio while a profile still carries one,
        // and the row is hidden when it does. Failing here would then block every
        // other setting in the profile with no way to clear the stale value short
        // of editing the configuration by hand.
        if handle.get_clock_propagation_ratio()?.is_none() {
            warn!("GPU has no adjustable GPC to XBAR ratio, ignoring the configured {percent}%");
            return Ok(());
        }
