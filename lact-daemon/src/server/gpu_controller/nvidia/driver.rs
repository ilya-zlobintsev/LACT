...
/// without one, and which of the two a card uses for GPC to XBAR is a property of
/// the board rather than of the architecture: an RTX 5090 reports every one of its
/// relationships as type 5 and so has no ratio to adjust, while an RTX PRO 6000 on
/// the same silicon, an RTX 5070 Ti and an RTX 2000 Ada all report type 3. There is
/// nothing to fall back to when a card reports type 5, so the control is simply not
/// offered there.
