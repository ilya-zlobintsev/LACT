...
///
/// The layout of the rest of the entry depends on it. Blackwell reports `0x10`,
/// while Ampere and Ada report `0x0b` with everything after the parent object
/// shifted four bytes back. Only the Blackwell layout is read here.
const CLK_DOMAIN_TYPE_BLACKWELL: u8 = 0x10;

/// Offset of the frequency offset range within a Blackwell info entry.
const BLACKWELL_RANGE_OFFSET: usize = 0x26;
...
impl ClkDomainInfo {
    ...
    /// other than Blackwell's. Ampere and Ada do report the same ±1000MHz ranges
    /// at a four byte lower offset, but nobody has confirmed that offsetting these
    /// domains has any effect there, so they are deliberately left out rather than
    /// exposed on the strength of the driver merely advertising a range.
    fn offset_range_mhz(&self) -> Option<(i32, i32)> {
        if self.obj_type != CLK_DOMAIN_TYPE_BLACKWELL {
            return None;
        }

        let at = BLACKWELL_RANGE_OFFSET - mem::offset_of!(Self, tail);
