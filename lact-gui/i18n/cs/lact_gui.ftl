...
acoustic-limit = Akustický limit (RPM)
acoustic-target = Akustický cíl (RPM)
min-fan-speed = Nejnižší přijatelné otáčky ventilátoru (%)
zero-rpm = Nula otáček
zero-rpm-stop-temp = Teplota (°C) pro zastavení nula otáček
...
oc-warning = Varování: změna těchto hodnot může vést k nestabilitě systému a případně poškodit váš hardware!
...
vram-pstate-clock-offset = Posun taktu (MHz) videopaměti GPU { $pstate }
...
pstates-manual-needed = Pozn.: aby bylo možné přepínat stavy napájení, je třeba výkonnostní úroveň nastavit na „ručně“
