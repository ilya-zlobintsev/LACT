...
gpu-clock-offset = Takt Offset (MHz)
max-gpu-voltage = Maximale Spannung (mV)
gpu-voltage-offset = Spannungs-Offset (mV)
...
pstates-manual-needed = Um die P-State Konfiguration ändern zu können, muss die Leistungsstufe auf „manuell“ eingestellt werden
...
no-sensors-found = Keine Sensoren gefunden
reset-now-button = Jetzt zurücksetzen
...
enable-vf-curve = Benutzerdefinierte VF Kurve
vram-section = VRAM
...
power-cap = Leistungslimit
power-section = Leistung
enable-locked-clocks = Gesperrte Takte
pstate-clock-offset = P-State { $pstate } Takt Offset (MHz)
