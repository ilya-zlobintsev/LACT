...
gpu-voltage-boost-tooltip = Voltage boost raises the maximum voltage the GPU is allowed to use, which can let it sustain higher clockspeeds. It has no effect when the card is already limited by its power or temperature limit, and it increases power draw and heat.
