...
reattach-page = Вернуться в главное окно
language-system-default = Стандартный для системы
show-page-window = Фокус на окне
page-detached = Эта страница открыта в другом окне.
min-clock = Минимальная частота (МГц)
detach-page = Открепить { $page }
language-restart-notice = Перезапустите графический интерфейс LACT, чтобы применить изменения языка.
