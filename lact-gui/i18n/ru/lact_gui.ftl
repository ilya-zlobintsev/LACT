...
spindown-delay = Задержка остановки вращения (мс)
speed-change-threshold = Порог изменения скорости (°C)
automatic-mode-threshold = Порог автоматического режима (°C)
spindown-delay-tooltip = Время, в течение которого температура GPU должна оставаться ниже заданного порога, прежде чем вентилятор снизит обороты
automatic-mode-threshold-tooltip =
    ...
    Эта настройка позволяет обойти это ограничение: при температуре выше заданного порога используется пользовательская кривая, а при более низкой — автоматический режим видеокарты с поддержкой Zero RPM.
